...
`;
	...
	try {
		...
		try {
			...
			// Groq with json_object mode usually returns clean JSON, but we'll parse safely
			ai_response = JSON.parse(aiGeneratedText);
		} catch (err) {
			throw new ApiError(
				300,
		...
		if (error.response) {
			console.error("AI API Response Data:", JSON.stringify(error.response.data, null, 2));
		}
...
const generateQuestion = asyncHandler(async (req, res) => {
	...
	const questionPromises = topics.map(async (topic) => {
		...
		**Rules:**
		...
		// Groq API adaptation for Questions
		const GROQ_API_KEY = process.env.GROQ_API_KEY;
		...
		const requestData = {
			...
			messages: [
				{
					role: "system",
					content: "You are a quiz generator. Always respond in valid JSON array format as requested."
				},
				{
					role: "user",
					content: prompt
