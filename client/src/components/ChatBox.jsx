		...
		try {
			...
			if (response.ok) {
				...
				if (data.message?.includes("429") || data.message?.includes("quota")) {
					throw new Error("API quota exceeded. Please try again later or check your API key.");
				}
			...
			let errorMessage = "⚠️ AI is not responding. ";
			
			if (error.message?.includes("quota")) {
				errorMessage += "API quota exceeded. Try again after 24 hours.";
			} else if (error.message?.includes("404")) {
				errorMessage += "AI model configuration error. Contact admin.";
			} else if (error.message?.includes("Failed to fetch")) {
				errorMessage += "Server is offline. Check if backend is running on port 8000.";
			} else {
				errorMessage += "Try again later or use Wikipedia/YouTube search.";
			}
